...
GasModel::GasModel(glm::ivec3 sz) : size_(sz) {
    ...
    gdm::qd([&] GDM_OP {
        ...
        for (int i = 0; i < 7; ++i) {
            ...
            std::cout << shader_srcs_[i] << std::endl;
            glCompileShader(shader_ids_[i]);

            prog_ids_[i] = glCreateProgram();
            glAttachShader(prog_ids_[i], shader_ids_[i]);
            glLinkProgram(prog_ids_[i]);
            std::cout << "compiling program" << i << std::endl;
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA,
                size_.x, size_.y, size_.z, 0, GL_RGBA, GL_FLOAT, vufs[i].data());
